...
public class OrdersUICollectionMore {
    ...
    public void add(){
        ...
        try {
            ...
            try {
                ...
                throw new InputMismatchException("Product value must be double!");
            }
            ...
            System.out.println("Error while adding order: "+ie.getMessage());
        } catch(InputMismatchException ime){
            System.out.println("Input typing error: "+ime.getMessage());
        }
    ...
    public void printBill(){
        ...
        for(String key: listNames ){
            ...
            for(Order o: list){
                listTotal += o.computeBill();
            }
            System.out.println("Total cost: "+listTotal);
            if ( listTotal>0.0 ) {
                // for(PaymentMethod pm: PaymentMethod.values()){
                //     System.out.println(pm + " fee: "+ pm.calculateTransactionFee(listTotal));
                // }
                // Scanner sc = new Scanner (System.in);
                // PaymentMethod pm=null;
                // do {
                //     System.out.print("How would you like to pay? Pls type CASH, DEBIT, CREDIT or TRANSFER: ");
                //     String n = sc.nextLine();
                //     pm = PaymentMethod.valueOf(n.toUpperCase()); // !!! Exception !!!
                // } while (pm==null);
                // System.out.println(pm + " fee: "+ pm.calculateTransactionFee(listTotal));
                List<PaymentMethod> availPMs = new ArrayList<PaymentMethod>();
                for (PaymentMethod pm: PaymentMethod.values()){
                    if( pm.calculateTransactionFee(listTotal)<listTotal )
                        availPMs.add(pm);
                }
                Collections.sort(availPMs, new TransactionFeeComparator(listTotal) );
                for(PaymentMethod pm: availPMs){
                    System.out.println(pm + " fee: "+ pm.calculateTransactionFee(listTotal));
                }
            }
        }
    ...
    public Order remove(){
        ...
        do {
            try {
                ...
                System.out.println("Index must be integer value!");
                sc.nextLine();
            }
